...
This runs Python linting, static typing, formatting checks, unit tests with coverage, the fake-backend extension acceptance suite, Swift formatting, a bridge build/version/capability smoke test, and Python/Swift builds. The acceptance assertions include exact album forwarding and private bridge files, partial-source continuation, strict no-swing exclusion without import, incremental cache hits and invalidation, rebuild, cancellation and guarded recovery, estimate failure, estimates above US$1, returned usage above estimate, original-upload rejection, metadata stripping, verified add-only import, post-success cleanup, explicit non-UI confirmation, and operation from unrelated project directories. Routine PhotoKit tests mock LaunchServices and verify exact-name forwarding, private bounded result files, cancellation/timeouts, sequential exports, checksums, partial failures, and verified import responses. PhotoKit authorization, real exact-album inventory, iCloud-backed export, and creation verification still require an explicitly approved signed app-bundle integration test on macOS because they depend on TCC and a real Photos library.
...
## 2026-09-03 release acceptance checkpoint

The package was installed from `git:github.com/nrgapple/swingcut@main` into Pi's global Git package store from an unrelated project directory. Pi registered `/swingcut-setup`; setup completed through Pi's RPC UI protocol after one destination confirmation, deployed the lockfile-backed wheel and signed helper, and activated the exact pushed revision. A release acceptance defect found during this check was fixed: macOS setup now replaces the `current` symlink without following the old release directory. A second defect was fixed by displaying the reviewed pricing-valid-through date alongside the estimate before confirmation.

An approved real `/swingcut` rebuild then ran from a second unrelated Pi project. The Pi flow presented one repeat-mode selection and one confirmation containing the exact album, aggregate count and duration, low-resolution silent proxy boundary, dated combined-path estimate, originals-local statement, rebuild mode, and add-only Photos destination. The run processed four sources, continued after two private per-source failures, retained three confident clips, rendered and verified the versioned Photos-compatible profile, created and verified one new Photos asset, and reached `succeeded`. Accounted provider usage was US$0.683599 against the disclosed US$0.969848 estimate. Provider uploads, staged sources, proxies, the local master, and the private terminal run record were absent after cleanup.

The final audit compared private runtime asset identifiers and filenames against every public Git blob without printing them and found no match. It also found no credential pattern in public history. The retained manifest contains only its 16 allowlisted aggregate/policy fields at mode `0600`; run directories are mode `0700`. Pi conversation-visible events contained no asset identifier, filename, source timestamp, local media path, or provider prose. `npm pack --dry-run` contained the package extension plus the backend/native/setup resources needed by Git installation, with no build environment or generated media.
