...
The signing wrapper temporarily prepends Swingcut's dedicated keychain to the user keychain search list because macOS otherwise cannot pair the trusted public certificate with its isolated private key. It restores the exact original search list in a `finally` block, including on ordinary termination signals, and never opens or changes another keychain.
