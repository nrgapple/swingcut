...
Swingcut pins video analysis to the Gemini Interactions API model `gemini-3.8-flash`, prompt `swing-analysis-v1`, agentic processing, and a strict JSON response schema. The provider accepts only a `ProxyArtifact` for `silent-h264-480w-15fps-v1` and re-probes and re-hashes its bytes immediately before upload. A missing sanitizer flag, changed hash, audio, excessive width, wrong frame rate/codec, or prohibited metadata blocks the upload.
...
The pricing snapshot is the official Gemini 3.8 Flash Standard paid-tier price through 2026-12-31: US$0.75 per million input tokens and US$3.75 per million output tokens, including thinking tokens. The snapshot expires closed on 2027-01-01.
...
The test generates the approved proxy in a temporary directory, performs the conservative preflight, uploads only that proxy, validates agentic analysis, and requires immediate provider-file deletion. Private paths, media, responses, and credentials must not be committed or copied into validation notes.
